...
### Removed
- **`Shares<TNumber>` is no longer marked `[Serializable]`**, and the two `[NonSerialized]` fields go with it. The attribute promised something the type could not deliver: the only serialized field holds `Share<TNumber>` instances, which are not marked serializable, so a formatter fails on the first element and only an *empty* collection could ever have been written — measured rather than reasoned, `typeof(Shares<T>).IsSerializable` was `true` while `typeof(Share<T>).IsSerializable` was `false`. What the attribute did deliver was an invitation to route secret-bearing objects through formatter-based serialization, which is obsolete on every modern target (SYSLIB0011, SYSLIB0050) and skips the validation this type performs in its constructor, because deserialization does not run one — the reason SonarQube reports `csharpsquid:S5766`, *Serializable objects should validate data during deserialization*, against that constructor. Persist shares through `ToCharArray(bool, bool, ShareFormat)` instead, the format this library defines and validates on the way back in. **Strictly this is a breaking change** for a consumer who binary-serialized an empty collection on .NET Framework; no other case worked. A test in both backend hierarchies now asserts the absence of the type flag, so re-adding it turns them red.
