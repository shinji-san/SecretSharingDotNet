...
### Changed
- `SecretReconstructor<TNumber, TExtendedGcdAlgorithm, TExtendedGcdResult>` and `SecretSplitter<TNumber>` no longer declare a finalizer. Neither owns an unmanaged resource, and the finalizer called `Dispose(false)`, which released nothing; its only effect was to put every instance on the finalization queue. A class deriving from `SecretReconstructor` that relied on the base finalizer to call its own `Dispose(false)` override must now declare a finalizer itself.
