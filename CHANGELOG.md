...
- Corrected and narrowed the constant-time claims for the `SecureBigInteger` backend in `README.md`, which ships inside the NuGet package README. The *Security & Threat Model* section stated that `Add`, `Subtract`, `Multiply`, `Square`, `Divide`, and `Remainder` depend only on the public operand bit length. Two value-dependent paths were missing from that account. First, every result is normalised through `TrimLeadingZerosInPlace`, whose scan stops at the first non-zero limb, so its iteration count depends on the result's magnitude — and because the trimmed length sizes the next operation, the limb count of an intermediate value is itself secret-derived rather than public. Second, `Add` and `Subtract` branch on whether the operand signs match, running structurally different work per branch; that mixed-sign branch is not taken on the Shamir values themselves, because the mark byte keeps them positive, but it *is* taken inside the modular inverse, where `MersenneSafeGcdAlgorithm` operates on the signed Bézout coefficients. The consequence for reconstruction is narrow: `Compute` has one call site, which passes the normalised denominator and the field prime, and both `DivMod` call sites build that denominator purely from differences of the public share indices — the secret share values enter the numerator, which never reaches `Compute`. At a fixed security level the divstep history is therefore a function of public data, and the warning binds callers using `SecureBigInteger` or `MersenneSafeGcdAlgorithm` directly on signed secret operands rather than the library's own reconstruction path. The same section now also lists three more value-dependent surfaces of `SecureBigInteger`: `ByteCount` (an early return on a zero high limb plus a `BytesInLimb` loop that counts significant bytes, evaluated on secrets, coefficients and share values), `CompareTo` (an early return on differing signs, ahead of the fixed-count comparison), and `ToByteArray` (several value-dependent early returns). Two further branches are documented. The sign routing of `Add` and `Subtract` runs in opposite directions: with equal signs, the ordinary case for Shamir values, `Subtract` takes the compare path and its magnitude ordering steers a branch (both selections do the same amount of work). And `Add`, `Subtract`, `Multiply`, `Divide` and `Remainder` branch on the result's zero status after the arithmetic — in `Add` and `Subtract` that branch reintroduces exactly the distinction the merged `comparison == 0` case was written to remove. The loop bound the 1.0.1 README stated was wrong as well: one bound, `max(left.LimbCount, right.LimbCount)`, was given for all six primitives, although it holds only for `Subtract` — `Add` iterates one limb more for the carry, `Multiply` and `Square` run nested `left × right` loops, and `Divide` and `Remainder` run `dividend.LimbCount × 64` outer iterations. Each bound follows from the limb counts rather than the digit values, so the constant-time property is unaffected, but the contract as written was materially incorrect. The sign branch is reached inside `MersenneSafeGcdAlgorithm.Compute`, not at each Lagrange division: `Compute` returns the Bézout coefficient already reduced into `[0, M_p)`, reducing two signed intermediates per call — `beta * inv2n` and `alpha * inv2n` — and taking the negative path for whichever of them is negative; that the path is *conditional* is what makes it observable at all. The normalisation cost scales with the result width, which exceeds the operand width — with two-limb operands a sum is three limbs and a product four. The guarantee is now stated for the per-limb loop only, every gap is listed explicitly, and the backend comparison table reports constant-time arithmetic as *partial* rather than *yes*. No library behaviour changed — this corrects documented security properties that were stated more broadly than the implementation supports.
...
[Unreleased]: https://github.com/shinji-san/SecretSharingDotNet/compare/v1.1.0...develop
