...
### Removed
- **`Shares<TNumber>` is no longer marked `[Serializable]`**; the two field-level `[NonSerialized]` attributes that only served it have been removed with it, while the fields themselves stay. The attribute promised something the type could not deliver: the only serialized field holds `Share<TNumber>` instances, which are not marked serializable, and a formatter rejects that element type **even when the collection is empty**, because the backing list carries a `Share<TNumber>[]` whose element type is checked regardless of its length. No instance was ever serializable — measured, not reasoned: `typeof(Shares<T>).IsSerializable` was `true`, `typeof(Share<T>).IsSerializable` was `false`, and `BinaryFormatter.Serialize` on an empty `Shares<BigInteger>` from the previous release threw `SerializationException` naming `Share<BigInteger>`. What the attribute did deliver was an invitation to route secret-bearing objects through formatter-based serialization, which is obsolete on every modern target (SYSLIB0011, SYSLIB0050) and skips the validation this type performs in its constructor, because deserialization does not run one — the reason SonarQube reports `csharpsquid:S5766`, *Serializable objects should validate data during deserialization*, against that constructor. Persist shares through `ToCharArray(bool, bool, ShareFormat)` instead, the format this library defines and validates on the way back in. **This is not a breaking change**: there was no working case to break. A test in both backend hierarchies now asserts that the attribute is absent, so re-adding it turns them red.
