...
### Deprecated
- The finalizer of `SecretReconstructor<TNumber, TExtendedGcdAlgorithm, TExtendedGcdResult>` is deprecated and will be removed in 2.0.0. It releases nothing of the base type's own; it exists so that a derived type reaches its `Dispose(false)` override when an instance is never disposed. A derived type that relies on this must declare its own finalizer from 2.0.0 on — **not before**: while the base finalizer exists, a derived finalizer chains to it and the override runs twice.
