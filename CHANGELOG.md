...
### Security
- **On `netstandard2.0`, `net472`, `net48` and `net481` the wipe cleared fewer bytes than the buffer holds.** `PinnedPoolArray<T>` sized it with `Marshal.SizeOf(typeof(T))`, which reports the *marshalling* size of a type rather than the size one element occupies in memory. For `char` that is 1 instead of 2, so a `PinnedPoolArray<char>` — the buffer behind `ConsolePasswordReader`, `ToPinnedSecure()` and every `ToCharArray()` result, holding secret text, Base64 and share strings — was overwritten across the first half of its bytes only and went back to `ArrayPool<char>.Shared` with the high byte of every character intact. For an enum `Marshal.SizeOf` throws instead of returning a size, so the wipe did not run at all. The length now comes from `sizeof(T)`, which is the layout of the managed array actually being wiped, and it is computed in 64 bit: at `int` width the product of length and element size overflows once the buffer reaches 2 GiB, wrapping to a smaller — often negative — number of bytes, so the wipe covered a fraction of the buffer or was skipped outright. Measured identically on .NET 8 and Mono 6.8. The modern targets were never affected — they take the byte span from `MemoryMarshal.AsBytes` and zero it through `CryptographicOperations.ZeroMemory`. No API change; `PinnedPoolArrayTest` pins the element size for `char`, `ulong` and an enum.
- **A failed wipe left the buffer pinned for the lifetime of the process, and could take the process down.** `PinnedPoolArray<T>.DisposeCore` wiped, then freed the `GCHandle`, then returned the array to the pool, none of it guarded. An exception out of the wipe — the enum case above, or `MemoryMarshal.AsBytes` refusing a span longer than `int.MaxValue` bytes on the modern targets — skipped the `GCHandle.Free`, leaving the array immobile with its contents in place, and escaped the finalizer, where an exception terminates the process. The pin is now released in a `finally`, the array is handed back to the pool only once the wipe has completed, and the finalizer swallows what it cannot report. A buffer kept out of the pool is the deliberate outcome: giving a partially wiped one to the next tenant is the worse one.
