...
### Changed
- `SecretSplitter<TNumber>` no longer declares a finalizer. It is sealed and owns no unmanaged resource, and the finalizer called `Dispose(false)`, which released nothing; its only effect was to put every instance on the finalization queue. `SecretReconstructor` keeps its finalizer for now: it is public and not sealed, and a derived type may rely on the finalizer to reach its own `Dispose(false)` override, so removing it is reserved for the next major version.
