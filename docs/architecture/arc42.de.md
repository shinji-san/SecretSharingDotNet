...
- Änderungen:
    ...
    - 2026-09-19 — Testzahlen auf 964 (759 `[Fact]` + 205 `[Theory]`) über 46 Testklassen
      aktualisiert: `SecurityLevelManagerTest` hat auf der `BigInteger`-Seite den
      Dispose-Idempotenztest bekommen, den bisher nur die `SecureBigInteger`-Seite hatte.
...
| 2 | **Funktionale Korrektheit des Schemas** | Ein falsch rekonstruiertes Geheimnis ist unbemerkt fatal: plain Shamir hat keine Integritätsprüfung (`README.md`, Threat Model). Abgesichert über 964 Testmethoden, property-basierte Round-Trip-Tests (CsCheck) und zwei parallele Testhierarchien für beide numerischen Backends. |
...
C4Container
  ...
  System_Boundary(sln, "SecretSharingDotNet.slnx") {
    ...
    Container(tests, "SecretSharingDotNetTest", "xUnit v3, Moq, CsCheck", "964 Testmethoden ueber 6 TFMs, inkl. Timing-Harness und Stress-Traits")
...
- **Jede Allokation bindet per `using`** — auch Operator-Ergebnisse (`+`, `-`, `*`, `/`, `%`),
  ...
  `using var`-Deklarationen auf 964 Testmethoden. Ein vergessenes `using` hält einen gepinnten
