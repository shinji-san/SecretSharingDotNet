...
| Die Betragsroutinen `AddUnsigned`, `SubtractUnsigned` und `CompareUnsigned` selbst — feste Limb-Anzahl, verzweigungsfreie Formeln | `Add` und `Subtract` verzweigen auf das **Vorzeichen** der Operanden: Bei gleichem Vorzeichen läuft `AddUnsigned`, bei gemischtem `CompareUnsigned` plus `SubtractUnsigned` — strukturell verschiedene Arbeit. Für die Shamir-Werte selbst (Geheimnis, Koeffizienten, Share-y-Werte) wird der Zweig nicht durchlaufen, weil das Markierungsbyte sie positiv hält und `MersenneModulo` in `[0, M_p)` reduziert. **Im Modularinversen dagegen schon:** `MersenneSafeGcdAlgorithm.ApplyExtendedDivstep` rechnet `newUG = uG - uF` auf den Bézout-Koeffizienten — ab `uG = 0`, `uF = 1` ergibt das bereits in der ersten Divstep-Iteration `−1`. Der gemischte Vorzeichenpfad läuft also bei jeder Rekonstruktion mit dem Safegcd, auf geheimnisabgeleiteten Werten. Dasselbe gilt für `MersenneModulo`, das einen eigenen Vorzeichenzweig trägt: Sein Negativpfad läuft drei zusätzliche Limb-Schleifen, ein `SubtractInPlace` und eine weitere gepinnte Allokation, die der Positivpfad überspringt — und `DivMod` reicht ihm bei **jeder** Lagrange-Division den vorzeichenbehafteten Bézout-Koeffizienten. `IsOne` kehrt bei negativem Vorzeichen ebenfalls vor seinem Fold zurück (Risiko R26). (`Multiply`, `Square`, `Divide` und `Remainder` sind davon **nicht** betroffen: Ihre Null-Prüfungen sind der Division-durch-Null-Vertrag auf dem Divisor bzw. eine O(1)-Vorzeichennormalisierung nach der Rechnung.) |
...
| **R26** | **Vorzeichenzweige werden auf geheimnisabgeleiteten Zwischenwerten durchlaufen.** `ApplyExtendedDivstep` rechnet auf den vorzeichenbehafteten Bézout-Koeffizienten (`newUG = uG - uF` ist ab der ersten Iteration negativ); `MersenneModulo` und `IsOne` tragen eigene Vorzeichenzweige, die `DivMod` bei jeder Lagrange-Division trifft. | Niedrig | Die Laufzeit unterscheidet gleich- von gemischtvorzeichigen Operanden auf geheimnisabgeleiteten Werten — bei jeder Rekonstruktion mit dem Safegcd. `MersenneModulo`s Negativpfad kostet drei zusätzliche Limb-Schleifen, ein `SubtractInPlace` und eine weitere Allokation. Reiht sich in die ohnehin dokumentierte Nicht-Uniformität je Divstep-Iteration ein. | Vorzeichenbehaftete Koeffizienten in Betrag plus separates Vorzeichenbit zerlegen und maskengesteuert rechnen — dieselbe Richtung wie die geparkte verzweigungsfreie Divstep-Variante. | `MersenneSafeGcdAlgorithm.ApplyExtendedDivstep` (Zeilen 497–511); `SecureBigInteger.MersenneModulo` (Negativzweig) und `IsOne`; `SecretReconstructor.DivMod` |
