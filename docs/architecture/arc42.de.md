...
|---|---|
| Kernarithmetik von `SecureBigInteger` (`Add`, `Subtract`, `Multiply`, `Square`, `Divide`, `Remainder`) — feste Limb-Anzahl `max(l, r)`, verzweigungsfreie Übertragsformeln. Die Garantie gilt für die **Beträge**: Gleiche Limb-Anzahl heißt gleiche Rechenarbeit, unabhängig von den Ziffernwerten. | `Pow(int)` — variabel über den *Exponenten* (der als öffentlich gilt) |
...
Die Nummerierung R1–R23 bleibt über Aktualisierungen hinweg stabil. Die Kennung **R12 ist nicht
