...
- Änderungen:
    ...
    - 2026-09-19 — das Löschen auf den vier Alt-Zielen nahm seine Länge aus `Marshal.SizeOf`, also
      der *Marshalling*-Größe: 1 für `char`, womit die Hälfte jedes Zeichens im gepoolten Puffer
      überlebte, und eine `ArgumentException` für ein Enum, womit gar nichts gelöscht wurde
      (gemessen auf .NET 8 wie auf Mono 6.8). Jetzt kommt die Länge aus `sizeof(T)` und wird in
      64 Bit gerechnet. Q1 und 8.1 halten den bisherigen Geltungsbereich und den Fix fest;
      Testzahlen auf 973 (768 `[Fact]` + 205 `[Theory]`) über 46 Testklassen aktualisiert — die
      drei Elementgrößen-Facts in `PinnedPoolArrayTest`.
...
| 2 | **Funktionale Korrektheit des Schemas** | Ein falsch rekonstruiertes Geheimnis ist unbemerkt fatal: plain Shamir hat keine Integritätsprüfung (`README.md`, Threat Model). Abgesichert über 973 Testmethoden, property-basierte Round-Trip-Tests (CsCheck) und zwei parallele Testhierarchien für beide numerischen Backends. |
...
C4Container
  ...
  System_Boundary(sln, "SecretSharingDotNet.slnx") {
    ...
    Container(tests, "SecretSharingDotNetTest", "xUnit v3, Moq, CsCheck", "973 Testmethoden ueber 6 TFMs, inkl. Timing-Harness und Stress-Traits")
...
- **Jede Allokation bindet per `using`** — auch Operator-Ergebnisse (`+`, `-`, `*`, `/`, `%`),
  ...
  `using var`-Deklarationen auf 973 Testmethoden. Ein vergessenes `using` hält einen gepinnten
