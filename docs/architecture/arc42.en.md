...
| **R26** | **Sign branches are exercised on secret-derived intermediates.** `ApplyExtendedDivstep` operates on the signed Bézout coefficients (`newUG = uG - uF` is negative from the first iteration on); `MersenneModulo` and `IsOne` carry sign branches of their own; `MersenneModulo`'s is reached inside `Compute` itself, which reduces two signed intermediates per call (`beta * inv2n` and `alpha * inv2n`) and takes the negative path for whichever is negative, rather than from `DivMod`. | Low | Timing separates equal-sign from mixed-sign operands on secret-derived values, on every reconstruction using the safegcd. `MersenneModulo`'s negative path costs three extra limb loops, a `SubtractInPlace` and one more allocation. It adds to the per-divstep non-uniformity already documented. | Split the signed coefficients into a magnitude plus a separate sign bit and drive the arithmetic by mask — the same direction as the parked branchless divstep variant. | `MersenneSafeGcdAlgorithm.ApplyExtendedDivstep` (lines 497-511); `SecureBigInteger.MersenneModulo` (negative branch) and `IsOne`; `SecretReconstructor.DivMod` |
