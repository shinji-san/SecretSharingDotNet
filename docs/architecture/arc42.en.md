...
| 5 | **Use-case interfaces as the public entry points** | `IMakeSharesUseCase<TNumber>` and `IReconstructionUseCase<TNumber>` extend `IDisposable` and are DI-friendly. Composition rather than inheritance is the intended extension direction, but only partly enforced: `SecretSplitter<TNumber>` and `FixedIterationSecretReconstructor<TNumber>` are `sealed`, while `SecretReconstructor<TNumber>`, its three-generic base and `SecurityLevelManager<TNumber>` are not — and `FixedIterationSecretReconstructor` itself derives from `SecretReconstructor<TNumber>`. Inheritance is available at those points and is used internally. |
...
| The magnitude routines `AddUnsigned`, `SubtractUnsigned` and `CompareUnsigned` themselves — fixed limb count, branchless formulas | `Add` and `Subtract` branch on the operands' **sign**: equal signs run `AddUnsigned`, mixed signs run `CompareUnsigned` plus `SubtractUnsigned` — structurally different work. For the Shamir values themselves (secret, coefficients, share y-values) the branch is not taken, because the mark byte keeps them positive and `MersenneModulo` reduces into `[0, M_p)`. **Inside the modular inverse it is taken:** `MersenneSafeGcdAlgorithm.ApplyExtendedDivstep` computes `newUG = uG - uF` on the Bézout coefficients, which from `uG = 0`, `uF = 1` yields `−1` already on the first divstep iteration. The mixed-sign path therefore runs on every reconstruction using the safegcd, on secret-derived values. The same holds for `MersenneModulo`, which carries a sign branch of its own: its negative path runs three additional limb loops, a `SubtractInPlace` and one more pinned allocation that the positive path skips — and `DivMod` feeds it the signed Bézout coefficient on **every** Lagrange division. `IsOne` likewise returns early on a negative sign, ahead of its fold (risk R26). (`Multiply`, `Square`, `Divide` and `Remainder` are **not** affected: their zero checks are the divide-by-zero contract on the divisor, or an O(1) sign normalisation after the computation.) |
