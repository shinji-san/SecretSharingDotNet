...
| Sign branch in `MersenneModulo`, reached inside `MersenneSafeGcdAlgorithm.Compute` when it reduces the signed intermediate `beta * inv2n` (R26) | The operand's sign | Three extra limb loops, a `SubtractInPlace` and one more pinned allocation. **Not** reached from `DivMod`: `Compute` returns the Bézout coefficient already reduced into `[0, M_p)` |
