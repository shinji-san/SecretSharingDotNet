...
|---|---|
| Core arithmetic of `SecureBigInteger` (`Add`, `Subtract`, `Multiply`, `Square`, `Divide`, `Remainder`) — fixed limb count `max(l, r)`, branchless carry/borrow formulas. The guarantee covers the **magnitudes**: equal limb counts mean equal work regardless of the digit values. | `Pow(int)` — variable on the *exponent* (which is treated as public) |
...
The numbering R1–R23 stays stable across updates. The identifier **R12 is unassigned**: it
